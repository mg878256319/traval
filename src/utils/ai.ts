...
- **每个需要住宿的晚上必须推荐一个具体的酒店**：酒店必须位于当天最后一个景点附近（就近住宿），方便步行或短途打车到达。如果行程跨区域（如从市区到景区），要换到离第二天活动近的酒店。不能连续几天推荐同一个酒店除非行程确实在同一区域
  - 豪华型（每间每晚≥1000元）：五星级/奢华度假酒店
  - 舒适型（每间每晚500-1000元）：四星级/高端精品酒店
  - 经济型（每间每晚250-500元）：三星级/商务酒店
  - 实惠型（每间每晚<250元）：经济型酒店/青旅/民宿
  每天推荐的酒店必须就近当天最后一个景点，包含酒店名称、价格、推荐理由、距景点距离
